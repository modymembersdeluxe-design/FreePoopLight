...
def render_project(config: Dict, out_path: str, progress_callback: Callable[[str], None] = None):
    ...
    try:
        ...
        with open(filelist, "w", encoding="utf-8") as fh:
            for p in processed:
                # ffmpeg concat demuxer requires absolute paths or relative; use safe escaping
                fh.write(f"file '{p.replace(\"'\",\"'\\''\")}'\n")
        ...
        if config.get("overlay"):
            ...
            tint_color = "0x00ff0080"  # green semi-transparent (alpha hex is not widely supported), so we'll use color source + blend
            # Use filter_complex to overlay a semi-transparent color generated by color source
            cmd_overlay = [
                "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
                "-i", tmp_out,
